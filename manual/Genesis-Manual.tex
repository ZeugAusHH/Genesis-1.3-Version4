...
\item {\bf write\_semaphore\_file} ({\it bool, false}): If {\tt true}, an empty file is generated as the simulation is completed. The file name is derived from {\tt rootname} and has the extension {\tt .sema}. No file is written if the simulation aborts (examples are errors while parsing the input file or errors while executing commands), or if the simulation crashes (for example because of out-of-memory condition).
